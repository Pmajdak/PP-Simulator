...
namespace SimConsole
{
    public class MapVisualizer
    {
        ...
        public void Draw()
        {
            Simulator.Point orcPos = new Simulator.Point(-1, -1);
            Simulator.Point elfPos = new Simulator.Point(-1, -1);
            Simulator.Point groupPos = new Simulator.Point(-1, -1);
            ...
            for (int x = 0; x < _map.SizeX; x++)
            {
                for (int y = 0; y < _map.SizeY; y++)
                {
                    if (_map.At(x, y).Count != 0)
                    {
                        ...
                        {
                            orcPos = new Simulator.Point(x, y);
                        }
                        if (_map.At(x, y).Count == 1 && _map.At(x, y)[0].ToString().StartsWith("E"))
                        {
                            elfPos = new Simulator.Point(x, y);
                        }
                        if (_map.At(x, y).Count > 1)
                        {
                            groupPos = new Simulator.Point(x, y);
                        }
                    }
                }
            }
